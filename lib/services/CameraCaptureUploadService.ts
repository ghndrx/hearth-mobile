import { cameraService, CapturedMedia, VideoQualityPreset } from './CameraService';
import { videoCompressionService, VideoCompressionOptions } from './VideoCompressionService';
...
export class CameraCaptureUploadService {
  ...
  async startUpload(sessionId: string, options: UploadOptions = {}): Promise<void> {
    ...
    // Store upload options for this session
    this.setSessionUploadOptions(sessionId, options);
  ...
  private async prepareMediaForUpload(session: CaptureUploadSession): Promise<void> {
    ...
    // For videos, compress using VideoCompressionService
    if (session.media.type === 'video') {
      const options = this.getSessionUploadOptions(session.id);
      ...
      if (options.compressVideo !== false) {
        try {
          ...
          // Compress the video
          const compressed = await videoCompressionService.compressVideo(
            session.media.uri,
            compressionOptions,
            (progress) => {
              // Map compression progress to overall progress (10-30% range)
              const mappedProgress = 10 + Math.floor(progress.percentage * 0.2);
          ...
          // Delete original temp file if compressed version is different
          if (compressed.uri !== session.media.uri) {
            await cameraService.deleteTempFile(session.media.uri);
          }

          // Update session with compressed media info
          session.media.uri = compressed.uri;
          session.media.fileSize = compressed.fileSize;
          session.media.width = compressed.width;
          session.media.height = compressed.height;
  ...
  /**
   * Get upload options for a session (helper to retrieve stored options)
   */
  private sessionUploadOptions = new Map<string, UploadOptions>();

  private getSessionUploadOptions(sessionId: string): UploadOptions {
    return this.sessionUploadOptions.get(sessionId) || {};
  }

  private setSessionUploadOptions(sessionId: string, options: UploadOptions): void {
    this.sessionUploadOptions.set(sessionId, options);
  }
